...
@section('contents')
    <div style="padding: 30px; padding-bottom: 50px;">
        @if(($sup->spayment_status ?? '') == 'N')
            @include('template.support-bank')
        @else
            @include('template.support-ok')
        @endif

        <form id="resend-frm" method="post" data-sid="{{ $sup->sid }}" data-case="resend-mail" style="margin-top: 65px;">
            <fieldset>
                <div class="sub-tit-wrap">
                    <h3 class="sub-tit">후원신청 메일 재발송</h3>
                    <p class="help-text">
