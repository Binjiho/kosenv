    ...
    <table class="cst-table list-table">
        ...
        @forelse($list as $row)
            <tr data-sid="{{ $row->sid }}">
                ...
                <td>
                    @if(($row->status ?? '') == 'Y')
                        <a href="{{ route('registration.receipt',['sid'=>$row->sid, 'work_code'=>$row->work_code]) }}" class="btn btn-small color-type5 call-popup" data-popup_name="receipt-pop" data-width="800" data-height="900">워드백업</a>
                    @endif
                </td>
                <td>
                    <a href="{{ route('abstract.modify', ['sid' => $row->sid, 'work_code'=>$row->work_code]) }}" class="btn-admin call-popup" data-name="abstract-upsert" data-width="1100" data-height="900" style="color: #5a5ad7;">
