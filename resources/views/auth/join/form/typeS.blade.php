    ...
    <li>
        ...
        <div class="form-con">
            <div class="radio-wrap cst">
                @foreach( $userConfig['grade']['S'] as $key => $val )
                <label class="radio-group" for="grade{{ $key }}">
                    <input type="radio" name="grade" id="grade{{ $key }}" value="{{ $key }}" {{ ( $user->grade ?? '' ) == $key ? 'checked' : '' }} >{{ $val }}
                </label>
                @endforeach
            </div>
        </div>
